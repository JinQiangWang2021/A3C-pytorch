...
class Worker(mp.Process):
    def __init__(self,env,continuous,state_size,action_size,id, global_valueNet,global_value_optimizer,
        ...
        self.local_policyNet = ActorDiscrete(self.state_size,self.action_size)
        if self.continuous:
            self.local_policyNet = ActorContinous(self.state_size,self.action_size)
        self.local_valueNet = ValueNetwork(self.state_size,1)
    ...
    def calculate_loss(self):
        ...
        states = torch.tensor([np.squeeze(t[0].numpy()) for t in self.memory], dtype=torch.float)
        action = torch.tensor([t[1] for t in self.memory], dtype=torch.float)
        ...
        policy_loss = (policy_loss + 0.05 * entropy).mean()
    ...
    def run(self):
        while self.global_epi.value < self.max_epi:
            ...
            while True:
                ...
                next_state, reward, done, _ = self.env.step(np.clip(action,-2,2))
                ...
                if done:
                    ...
                    print("w{} | episode: {}\t , episode reward:{:.4} \t  ".format(self.name,self.global_epi.value,self.global_epi_rew.value))
                    break
